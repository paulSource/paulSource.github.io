...
function equal() { // функция вычисляет значение того что находится в textview, возвращает в textview результат, !!NEW Добавляет строку вычисления в историю
    ...
    if (result !== 'error') result = calcStr(task);
    historyAdd(document.form.textview.value, result); //добавляем строку в historyView
    document.form.textview.value = result; //выводим результат на поле
}
...
function historyAdd(task, result) {
